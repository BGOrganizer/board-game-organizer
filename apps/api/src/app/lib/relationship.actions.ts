...
export const REMOVE: Record<string, Action> = {
  ...
  friend_request: (userId, targetUserId, repo) => repo.clearBidirectional(userId, targetUserId, ['friend_request']),
  friend: (userId, targetUserId, repo) => repo.clearBidirectional(userId, targetUserId, ['friend', 'friend_request']), // follow preservato
