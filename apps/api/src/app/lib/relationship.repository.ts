...
import { Relationship, RelationshipType, RelationshipStatus } from '@/app/models/relationship'
...
export class RelationshipRepository {
  ...
  async isBlocked(a: string, b: string) {
    const count = await this.col.countDocuments(
      { type: 'block', $or: [{ fromUserId: a, toUserId: b }, { fromUserId: b, toUserId: a }] },
      this.opts
    )
    return count > 0
  }
  ...
  async list(userId: string, type: RelationshipType, status: RelationshipStatus, direction: Direction) {
  ...
  async getRelationshipStatus(viewerId: string, targetId: string) {
    ...
    if (blockedByTarget) {
      return { isFollowing: false, isFriend: false, friendRequestSent: false, friendRequestReceived: false, hasBlocked: false, isBlockedBy: true }
    }
