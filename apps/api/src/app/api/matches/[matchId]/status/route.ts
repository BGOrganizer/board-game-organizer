import { setMatchStatusSchema } from "@board-game-organizer/schemas";
import {
  badMatchRequest,
...
export async function PATCH(request: Request, context: MatchRouteContext) {
  ...
  const parsed = setMatchStatusSchema.safeParse(body.data);
  ...
  return runMatchOperation(request, async ({ userId, service }) => ({
    match: await service.setStatus(userId, matchId, parsed.data.status),
  }));
}
